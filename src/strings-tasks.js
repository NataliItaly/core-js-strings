...
function isPalindrome(str) {
  const original = str.split('').filter((char) => /[a-z]/gi.test(char));
  return (
    original.join('').toLowerCase() ===
    [...original].reverse().join('').toLowerCase()
  );
}
...
function encodeToRot13(str) {
  const shiferStr1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const shiferStr2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((char) => {
      if (shiferStr1.includes(char)) {
        return shiferStr2[shiferStr1.indexOf(char)];
      }
      if (shiferStr2.includes(char)) {
        return shiferStr1[shiferStr2.indexOf(char)];
      }
      return char;
    })
    .join('');
